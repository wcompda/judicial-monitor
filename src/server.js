...
app.post('/api/solicitar-cadastro', async (req, res) => {
  try {
    ...
    const html = `<div style="font-family:Arial,sans-serif;max-width:500px;margin:0 auto"><div style="background:#1e40af;padding:20px;border-radius:12px 12px 0 0;text-align:center"><h2 style="color:white;margin:0">🏛️ JudicialMonitor</h2><p style="color:rgba(255,255,255,.85);margin:6px 0 0">Bem-vindo ao Monitor Judicial</p></div><div style="background:white;padding:24px;border-radius:0 0 12px 12px"><p>Olá, <strong>${nome}</strong>!</p><p>Sua solicitação de acesso foi recebida. Clique no botão abaixo para completar seu cadastro:</p><a href="${link}" style="display:block;background:#1e40af;color:white;text-align:center;padding:14px;border-radius:8px;text-decoration:none;font-weight:bold;font-size:16px;margin:20px 0">Completar Cadastro</a><p style="color:#64748b;font-size:12px">Este link expira em 24 horas. Se não solicitou acesso, ignore este email.</p></div></div>`;
    const https = require('https');
    const body = JSON.stringify({ from: 'JudicialMonitor <onboarding@resend.dev>', to: [email], subject: '🏛️ Complete seu cadastro — JudicialMonitor', html });
